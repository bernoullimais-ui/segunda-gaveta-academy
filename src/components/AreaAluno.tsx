          ...
          <div className="flex items-center justify-between h-20">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="hidden sm:flex flex-col items-end mr-2">
                <span className="text-sm font-bold text-slate-900 leading-none mb-1">{loggedUser.nome || loggedUser.email}</span>
                <span className="text-[10px] uppercase font-black text-slate-400 tracking-widest">{userRole}</span>
              </div>
  ...
  return (
    <button
      ...
      className={`px-4 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 transition-all relative ${
        active 
          ? 'bg-white text-indigo-600 shadow-sm shadow-indigo-100 ring-1 ring-indigo-50' 
    ...
    >
      ...
      {badge !== undefined && (
        <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] flex items-center justify-center bg-red-500 text-white text-[10px] font-black rounded-full px-1 border-2 border-slate-50">
          {badge}
        </span>
      )}
    </button>
